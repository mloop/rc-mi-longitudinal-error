...
$$E[W_{F,j}^N] = \gamma_0 + W_{F,j}^O\gamma_1$$

After fitting this model, we estimate $\hat{E[W_{i,B}^N]}$ for $i = 1,\dots,n$.^[This moment is where the approach we detail here departs from the classic regression calibration approach in measurement error studies. In classic studies, all subjects have $W$ and some subjects have $X$. The regression equation is then $E[X_j] = \gamma_0 + W_j\gamma_1$. Then, $\hat{E[X_j]}$ is used as the exposure for all participants in the health outcome regression that you are actually trying to study. In our study, instead of calibrating the error-prone measurement to the true measurement, we are calibrating one error-prone measurement to a different error-prone measurement.] In other words, we use the regression calibration model above to predict what the measurements on the new device would have looked like for the participants at baseline, who all got measurements only with the old device. The analysis set is now $i = 1,\dots,n$, where $n$ is the total number of participants who participated in baseline and follow up.

$$\hat{W}_{B,i}^N = \hat{\gamma_0} + W_{B,i}^O\hat{\gamma_1}$$

$\Delta W_{j,centered} = \Delta W_j - \overline{\Delta W_j}$, where $\Delta W_j = (W_{j,F}^O - W_{j,B}^O)$

Exposure: $\Delta W_{i,centered} = \Delta W_i - \overline{\Delta W_i}$, where $\Delta W_i = (W_{i,F}^N - \hat{E[W_{i,B}^N]})$
